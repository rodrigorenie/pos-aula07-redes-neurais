...
mnist_trainset = datasets.MNIST(root='data', train=True, download=True,
                                transform=transform)
train_loader = t.utils.data.DataLoader(mnist_trainset, batch_size=10,
                                       shuffle=True)

mnist_testset = datasets.MNIST(root='data', train=False, download=True,
                               transform=transform)
test_loader = t.utils.data.DataLoader(mnist_testset, batch_size=10,
                                      shuffle=True)
...
for epoch in range(epoch):
    ...
    epoch_error = epoch_loss / len(train_loader)
    loss_values.append(epoch_error)
    print(f'[{time.time() - timeit:02.5f}s] '
          f'erro epoca {epoch:02}: {epoch_error:02.10}')
